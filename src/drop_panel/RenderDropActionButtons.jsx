...
export default function RenderDropActionButtons({ markerRef, comment, setComment, gifState, setGifState, setShowInput, setIsDropLocked }) {
  const handleDrop = () => {
    ...
    let html = '<div style="display: grid; grid-template-columns: auto 1fr; gap: 8px; padding: 4px; align-items: center;">';
    if (gifUrl) {
      html += `<img src="${gifUrl}" style="max-width: 55px; border-radius: 8px; box-shadow: 0 4px 20px rgba(0,0,0,0.5); border: 1px solid rgba(255,255,255,0.1);"/>`;
    }
